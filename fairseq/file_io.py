...
class PathManager:
    ...
    def get_local_path(path: str, **kwargs) -> str:
        if path.startswith("hdfs://"):
            if path in PathManager.local_cache:
                return PathManager.local_cache[path]
            local_path = "/tmp/{}_{}".format(time.time(), random.randint(1, 200000))
            gf.copy(path, local_path)
            print("load {} to {}".format(path, local_path))
            PathManager.local_cache[path] = local_path
            return local_path
        return path
    ...
    def copy(src_path: str, dst_path: str, overwrite: bool = False) -> bool:
        if src_path.startswith("hdfs://"):
            if overwrite:
                gf.copy(src_path, f"{dst_path}.COPYING", True)
                gf.rename(f"{dst_path}.COPYING", dst_path, True)
            else:
                gf.copy(src_path, dst_path, False)
            return True
        return shutil.copyfile(src_path, dst_path)

    @staticmethod
    def rename(src: str, dst: str, overwrite: bool = True):
        if src.startswith("hdfs://"):
            gf.rename(src, dst, overwrite)
            return
        os.rename(src, dst)
