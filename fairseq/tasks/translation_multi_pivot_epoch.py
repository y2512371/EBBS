...
class SynchronousIterator:
    ...
    def __next__(self):
        ...
        if len(results) == len(self.iterators):
            return tuple(results)
        else:
            raise StopIteration

    def next_epoch_itr(self, shuffle=False):
        self.iterators = [x.next_epoch_itr(shuffle=shuffle) for x in self.iterators]
        return self
...
@register_task("translation_multi_pivot_epoch")
class TranslationMultiPivotEpochTask(LegacyFairseqTask):
    """
    ...
    def add_args(parser):
        ...
        parser.add_argument('--pivot-langs', nargs='*', type=str, help='A list of paths to pivot languages')
        parser.add_argument('--pivot-datadir', type=str, default=None, help='Path to the pivot datasets')
    ...
    def __init__(self, args, langs, dicts, training):
        ...
        self.pivot_datadir = args.pivot_datadir
        self.pivot_langs = args.pivot_langs

        for pivot_lang in args.pivot_langs:

            _args = copy.deepcopy(args)
            _args.source_lang = pivot_lang
            _langs, _dicts, _training = MultilingualDatasetManager.prepare(
                TranslationMultiPivotEpochTask.load_dictionary, _args,
            )
            dicts[pivot_lang] = _dicts[pivot_lang]

        self.pivot_data_managers = {}
        for pivot_lang in args.pivot_langs:
            _args = copy.deepcopy(args)
            _args.data = str(PosixPath(args.pivot_datadir, f'{args.source_lang}-{pivot_lang}-{args.target_lang}', f'pivot_{pivot_lang}2{args.target_lang}', 'data-bin'))
            manager = MultilingualDatasetManager.setup_data_manager(
                _args, [f'{pivot_lang}-{args.target_lang}'], langs, dicts, self.sampling_method
            )
            self.pivot_data_managers[pivot_lang] = manager
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        for pivot_lang in self.pivot_langs:
            _kwargs = kwargs
            self.datasets[f'{split}_pvt={pivot_lang}'] = self.pivot_data_managers[pivot_lang].load_dataset(
                split,
                self.training,
                epoch=epoch,
                combine=combine,
    ...
    def build_generator(
        ...
        # Copied from fairseq_task.py
        # Originally initialized using:
        ...
        # Default args for super().build_generator
        seq_gen_cls=None
        ...
        from fairseq.pivot_ensemble_generator import (
            PivotEnsembleGenerator,
            PivotVotingGenerator,
        )
        ...
        if seq_gen_cls is None:
            if getattr(args, "print_alignment", False):
                ...
                if extra_gen_cls_kwargs['ensemble_mode'] == 'averaging':
                    del extra_gen_cls_kwargs['ensemble_mode']
                    seq_gen_cls = PivotEnsembleGenerator
                elif extra_gen_cls_kwargs['ensemble_mode'] == 'voting':
                    del extra_gen_cls_kwargs['ensemble_mode']
                    del extra_gen_cls_kwargs['averaging_mode']
                    seq_gen_cls = PivotVotingGenerator
                else:
                    raise NotImplementedError()
        ...
        with torch.no_grad():
            ...
            if not self.args.lang_tok_replacing_bos_eos:
                if prefix_tokens is None and tgt_langtok_spec:
                    ...
                    src_tokens = sample[0]["net_input"]["src_tokens"]
    ...
    def get_batch_iterator(
        ...
        main_iter = iterators.EpochBatchIterator(
            dataset=dataset,
        ...
        iters = [main_iter]
        for pvt_lang in self.pivot_langs:
            pvt_iter = iterators.EpochBatchIterator(
                dataset=self.datasets[f'test_pvt={pvt_lang}'],
                collate_fn=self.datasets[f'test_pvt={pvt_lang}'].collater,
                batch_sampler=lambda x, y: main_iter.frozen_batches,
                seed=seed,
                num_shards=num_shards,
                shard_id=shard_id,
                num_workers=num_workers,
            ...
            iters.append(pvt_iter)

        epoch_iter = SynchronousIterator(iters)
