...
from fs_plugins.data.nat_datasets_mod_length_filtering import NATLanguagePairDataset as LanguagePairDataset
from fs_plugins.data.nat_datasets_mod_length_filtering import NATConcatDataset as ConcatDataset
from fs_plugins.data.nat_datasets_mod_length_filtering import NATPrependTokenDataset as PrependTokenDataset
from fs_plugins.data.nat_datasets_mod_length_filtering import NATTransformEosLangPairDataset as TransformEosLangPairDataset
from fs_plugins.data.nat_datasets_mod_length_filtering import NATTruncateDataset as TruncateDataset
...
from fs_plugins.data.nat_sampled_multi_dataset import NATSampledMultiDataset
from fs_plugins.data.nat_sampled_multi_epoch_dataset import NATSampledMultiEpochDataset
...
class NATMultilingualDatasetManager(object):
    ...
    def setup_data_manager(cls, args, lang_pairs, langs, dicts, sampling_method):
        return NATMultilingualDatasetManager(
    ...
    def get_any_dictionary(self):
        if self.args.target_dict is not None:
            return self.dicts[TGT_DICT_NAME]
        else:
            return self.dicts[list(self.dicts.keys())[0]]
    ...
    def get_all_decoder_langtok(self, spec=None):
        if spec is None:
            return None
        lang_tok_list = []
        for tgt_lang in self.tgt_langs:
            ...
            lang_tok_list.append(self.get_langtok_index(langtok, self.get_any_dictionary())) # NOTE: hack, only work for shared vocab
        return lang_tok_list

    def get_all_decoder_langtok_as_dict(self, spec=None):
        if spec is None:
            return None
        lang_tok_list = {}
        for tgt_lang in self.tgt_langs:
            ...
            lang_tok_list[tgt_lang] = self.get_langtok_index(langtok, self.get_any_dictionary()) # NOTE: hack, only work for shared vocab
        return lang_tok_list
    ...
    def load_langpair_dataset(
        ...
        return LanguagePairDataset(
            self.args.skip_src_langtok,
            self.args.replace_src_langtok_with_tgt,
            src_dataset,
            src_dataset.sizes,
            src_dict,
            tgt_dataset,
    ...
    def tgt_dataset_tranform_func(self, source_lang, target_lang, dataset, spec=None):
        ...
        if spec == 'bos':
            return PrependTokenDataset(dataset, self.lang_dict.bos_index)
        if not spec:
            return dataset
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        enable_lang_ids = self.args.enable_lang_ids # True # NOTE: force self.args.enable_lang_ids to be True
    ...
    def load_sampled_multi_epoch_dataset(
        ...
        if training and split == getattr(self.args, "train_subset", None):
            ...
            return NATSampledMultiEpochDataset(
                OrderedDict(datasets),
                epoch=epoch,
