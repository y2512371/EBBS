...
@register_task("translation_multi_simple_epoch_bleu_bt")
class TranslationMultiSimpleEpochTaskBLEUBT(TranslationMultiSimpleEpochTask):
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        # model.generator = self.build_generator_for_bleu([model], args)
        return model
    ...
    def inference_step_top1(
        ...
        with torch.no_grad():
            ...
            if not self.args.lang_tok_replacing_bos_eos:
                ...
                gen = [x[0]['tokens'] for x in gen]
                gen_len_list = [len(x) for x in gen]
                max_len = max(gen_len_list)
                min_len = min(gen_len_list)
                if max_len > min_len:
                    gen2 = [torch.cat((x, torch.tensor([1]*(max_len-len(x))).to(x)), dim=-1) 
                                if len(x) < max_len else x for x in gen]
                    return torch.stack(gen2, dim=0)
                else:
                    return torch.stack(gen, dim=0)
            else:
                gen = generator.generate(
                    models,
                    sample,
